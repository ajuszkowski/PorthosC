...
import mousquetaires.memorymodels.relations.BasicRelation;
import mousquetaires.memorymodels.axioms.CandidateAxiom;
import mousquetaires.memorymodels.relations.RelInterSect;
import mousquetaires.memorymodels.relations.RelUnion;
import mousquetaires.memorymodels.relations.Relation;
import mousquetaires.memorymodels.relations.TemplateRelation;
...
public class ListOfRels {
    ...
    protected void addCandidates(boolean isCEGIS) {
        if (isCEGIS) {
            ...
            //call CEGIS loop:
            addCandidatesCEGIS(currentprogs);
            s.pop();
        } else {
            addCandidatesSimple();
        }
     ...
     */
    private void addCandidatesCEGIS(Set<Integer> currentProgs) {
        //choose a program in NEG that we want to fail.
        for (int i = 0; i < Aramis.negPrograms.size(); i++) {
            if (!currentProgs.contains(i)) {
                log.fine("Processing NEG program " + Aramis.negPrograms.get(i));
                currentProgs.add(i);
                            ...
                            for (Event e2 : events) {
                                for (String rel : baserels) {
                                    ...
                                    if (m.eval(relPair, true).isTrue()) {
                                        ...
                                        Aramis.solCEGIS.add(relNamedPair);
                                    } else {
                                     //exec = exec + " " + String.format("n%s(%s,%s)", rel, e1.repr(), e2.repr());
                                     //TODO: remove?
                                     Aramis.solCEGIS.add(Aramis.ctx.mkNot(relNamedPair));
                                    }
                                }
                            }
                        }
                        log.info(exec);
                        if(exec.contentEquals(oldexec)) System.err.println("Execution didnt change!");
                        oldexec = exec;
