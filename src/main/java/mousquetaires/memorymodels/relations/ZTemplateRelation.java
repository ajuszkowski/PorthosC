...
public static String PREFIX="";
    ...
    public static ZTemplateRelation getTemplateRelation(int level) {
        if(level>1) return new ZTemplateRelation(getTemplateRelation(level-1), getTemplateRelation(level-1));
        else return new ZTemplateRelation(new ZTemplateBasicRelation(), new ZTemplateBasicRelation());
    }

    public ZRelation getSolution(Solver s, Context ctx){
        Model m=s.getModel();
        ZRelation rel1,rel2;
        if(r1 instanceof ZTemplateRelation)rel1=((ZTemplateRelation) r1).getSolution(s, ctx);
        else rel1=((ZTemplateBasicRelation)r1).getSolution(s, ctx);
        if(r2 instanceof ZTemplateRelation)rel2=((ZTemplateRelation) r2).getSolution(s, ctx);
        else rel2=((ZTemplateBasicRelation)r2).getSolution(s, ctx);
