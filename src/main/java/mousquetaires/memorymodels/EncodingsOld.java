...
public class EncodingsOld {
    ...
    public static BoolExpr encodeCommonExecutions(Program p1, Program p2, Context ctx) throws Z3Exception {
        BoolExpr enc = ctx.mkTrue();
        Set<Event> lEventsP1 = p1.getEvents().stream().filter(e -> e instanceof SharedMemEvent | e instanceof LocalEvent).collect(Collectors.toSet());
        Set<Event> lEventsP2 = p2.getEvents().stream().filter(e -> e instanceof SharedMemEvent | e instanceof LocalEvent).collect(Collectors.toSet());
        Set<Event> rEventsP1 = p1.getEvents().stream().filter(e -> e instanceof LoadEvent).collect(Collectors.toSet());
        Set<Event> wEventsP1 = p1.getEvents().stream().filter(e -> e instanceof StoreEvent || e instanceof InitEvent).collect(Collectors.toSet());
        Set<Event> rEventsP2 = p2.getEvents().stream().filter(e -> e instanceof LoadEvent).collect(Collectors.toSet());
        Set<Event> wEventsP2 = p2.getEvents().stream().filter(e -> e instanceof StoreEvent || e instanceof InitEvent).collect(Collectors.toSet());
    ...
    public static BoolExpr encodeReachedState(Program p, Model model, Context ctx) {
        Set<Location> locs = p.getEvents().stream().filter(e -> e instanceof SharedMemEvent).map(e -> e.getLoc()).collect(Collectors.toSet());
        ...
        Set<Register> regs = executedEvents.stream().filter(e -> e instanceof LocalEvent | e instanceof LoadEvent).map(e -> e.getReg()).collect(Collectors.toSet());
