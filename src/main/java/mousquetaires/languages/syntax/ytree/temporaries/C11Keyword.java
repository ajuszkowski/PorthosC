//package mousquetaires.languages.structure.ytree.types;
//
//import mousquetaires.languages.toytree.YEntity;
//
//import java.util.EnumMap;
//import java.util.Map;
//
//
///**
// * Note, this class implements YEntity, although it does not belongs to the returnType system of Internal language.
// * This is done in order to simplify parsing types in CminToInternalTransformer visitor, where this C11-returnType will be
// * converted to Internal-returnType on the last stage, when the assignee identifier is determined.
// */
//public enum CminKeyword implements YEntity {
//    // typeSpecifier:
//    Signed,
//    Unsigned,
//    // variableTypeQualifier:
//    Const,
//    Restrict,
//    Volatile,
//    Atomic,
//    // primitiveTypeKeyword:
//    Void,
//    Char,
//    Short,
//    Int,
//    Long,
//    LongLong,
//    Float,
//    Double,
//    LongDouble,
//    Bool,
//    ;
//
//    /**
//     * https://stackoverflow.com/a/697531
//     * Type:      C99 Minimum:
//     * char       8
//     * short      16
//     * int        16 (however it's always 32)
//     * long       32
//     * long long  64
//     * float      32
//     * double     64
//     * long double 80 (64)
//     * see possible set of modifiers: https://en.wikipedia.org/wiki/C_data_types
//     */
//    private final static Map<CminKeyword, YPrimitiveType> bitnessMap =
//            new EnumMap<CminKeyword, YPrimitiveType>(CminKeyword.class) {{
//        put(Void,       YPrimitiveType.i0); // todo: set with respect to target machine bitness
//        put(Bool,       YPrimitiveType.i1);
//        put(Char,       YPrimitiveType.i8);
//        put(Short,      YPrimitiveType.i16);
//        put(Int,        YPrimitiveType.i16); // todo: set with respect to data model, often int is 32-bit!
//        put(Long,       YPrimitiveType.i32);
//        put(LongLong,   YPrimitiveType.i64);
//        put(Float,      YPrimitiveType.i32);
//        put(Double,     YPrimitiveType.i64);
//        put(LongDouble, YPrimitiveType.i64);
//    }};
//
//    public static YPrimitiveType convertOrNull(CminKeyword keyword) {
//        return CminKeyword.bitnessMap.get(keyword);
//    }
//
//    public static YPrimitiveType convert(CminKeyword keyword) {
//        YPrimitiveType result = convertOrNull(keyword);
//        if (result == null) {
//            throw new IllegalArgumentException(keyword.id());
//        }
//        return result;
//    }
//}
