...
public class UnrollerThriceNestedLoopTest extends UnrollerTestBase {
    ...
    protected IntTestFlowGraph getNonUnrolledGraph() {
        ...
        IntGraphNode source = node(0), sink = node(-1);
        IntTestFlowGraphBuilder builder = new IntTestFlowGraphBuilder(source, sink);
        builder.addPath(true, source, node(1), node(2), node(3));
        builder.addEdge(true,  node(5), node(1));
        builder.addEdge(false, node(5), sink);

        builder.addEdge(true,  node(4), node(2));
        builder.addEdge(false, node(4), node(5));

        builder.addEdge(false, node(3), node(4));
        builder.addEdge(true, node(3), node(1));
